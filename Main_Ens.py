...
def limit_memory(max_memory_mb):
    ...
    try:
        ...
        if max_memory_bytes > hard:
            max_memory_bytes = hard  # Ensure we don't set a limit higher than the hard limit
        resource.setrlimit(resource.RLIMIT_AS, (max_memory_bytes, hard))
        print(f"Memory limit set to {max_memory_mb} MB.")
    except ValueError:
        print("Memory limit could not be set, proceeding without memory limit.")

def track_memory(stage=""):
    """Tracks the memory usage at a given stage."""
    process = psutil.Process(os.getpid())
    mem_info = process.memory_info()
    rss_memory = mem_info.rss / (1024 ** 2)  # Convert to MB
    print(f"[{stage}] Memory Usage: {rss_memory:.4f} MB")
    return rss_memory
...
x_mb = int(sys.argv[2])  # Memory limit in MB
...
def run_classifier_three_times(classifier_func, *args):
    ...
    initial_memory = track_memory(f"Initial {classifier_func.__name__} Memory")  # Memory before running the classifier
    ...
    final_memory = track_memory(f"After {classifier_func.__name__} Memory")  # Memory after running the classifier
    memory_used_gb = (final_memory - initial_memory) / 1024  # Convert MB to GB
    print(f"Memory Used by {classifier_func.__name__}: {memory_used_gb:.4f} GB\n")
